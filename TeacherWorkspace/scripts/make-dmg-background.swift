...
// dark theme) and emits bg.png + bg@2x.png + a combined HiDPI bg.tiff into the
...
func render(scale: CGFloat) -> CGImage {
    ...
    ctx.setFillColor(hex(0x191917))
    ctx.fill(CGRect(x: 0, y: 0, width: W, height: H))
    let blue = CGGradient(colorsSpace: space,
                          colors: [hex(0x7D9BEB, 0.13), hex(0x7D9BEB, 0)] as CFArray,
                          locations: [0, 1])!
    ...
    let amber = CGGradient(colorsSpace: space,
                           colors: [hex(0xD6863A, 0.05), hex(0xD6863A, 0)] as CFArray,
                           locations: [0, 1])!
    ...
    for x: CGFloat in [165, 495] {
        ...
        ctx.addPath(path)
        ctx.setFillColor(hex(0xFFFFFF, 0.035))
        ctx.fillPath()
        ctx.addPath(path)
        ctx.setStrokeColor(hex(0xFFFFFF, 0.07))
        ctx.setLineWidth(1)
        ctx.strokePath()
    }
